...
class AbstractParser(b3.parser.Parser):
    ...
    def getAdmin(self, admin=None):
        """
        Get admin GUID or UID
        :param admin: The admin who performed action
        """
        if not admin:
            return " - Admin GUID: -1"
        else:
            if admin.pbid:
                return ' - Admin UID: @' + admin.pbid
            else:
                return ' - Admin GUID: ' + admin.guid[-8:]
    ...
    def kick(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        """
        text = self.getAdmin(admin)
        if isinstance(client, basestring) and re.match('^[0-9]+$', client):
            self.write(self.getCommand('kick', cid=client, reason=reason + text))
            return

        if self.PunkBuster:
            self.PunkBuster.kick(client, 0.5, reason)
        else:
            self.write(self.getCommand('kick', cid=client.cid, reason=reason + text))
            self.write(self.getCommand('message', cid=admin.cid, message='You just kicked player %s' % client.exactName))		
    ...
    def ban(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        if self.PunkBuster:
            ...
            self.write(self.getCommand('ban', cid=client.cid, reason=reason + text))
            self.write(self.getCommand('message', cid=admin.cid, message='You just banned player %s' % client.exactName))
    ...
    def tempban(self, client, reason='', duration=2, admin=None, silent=False, *kwargs):
        ...
        if isinstance(client, b3.clients.Client) and not client.guid:
            ...
            self.write(self.getCommand('tempban', cid=client, reason=reason + text))
            return
        else:
            banduration = b3.functions.minutesStr(duration)
        ...
        if self.PunkBuster:
            ...
            self.write(self.getCommand('tempban', cid=client.cid, reason=reason + text, duration=duration))
            self.write(self.getCommand('message', cid=admin.cid, message='You just tempbanned player %s for %s' % (client.exactName, banduration)))
