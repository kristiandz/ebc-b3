...
class Cod4Parser(b3.parsers.cod2.Cod2Parser):
    ...
    _commands = {
        ...
        'kickbyfullname': 'kick %(cid)s %(reason)s'
    }
    ...
    def unban(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        if self.PunkBuster:
            ...
            name = self.unban_g_uid(client) #Swat
            name2 = self.unban_g2_uid(client) #Swat
            result = self.write(self.getCommand('unban', name=name, reason=reason))
            result2 = self.write(self.getCommand('unban', name=name2, reason=reason))
            if admin:
                admin.message('^1You Have Unbanned ^2Player ^4%s' % client.exactName)
...
##############################################################################
##             Swat (GameRanger Account Unbanning Bug Fix!) Swat            ##
##############################################################################

    def unban_g_uid(self, client):
        if client.pbid:
            return '@' + client.pbid
        else:
            return client.guid[-8:]

    def unban_g2_uid(self, client):
        if client.pbid:
            return '@' + client.pbid
        else:
            return '@2' + client.guid[-8:]
