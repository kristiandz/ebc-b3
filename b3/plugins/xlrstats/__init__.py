...
class XlrstatsPlugin(b3.plugin.Plugin):
    ...
    _default_messages = {
        'cmd_xlrstats': '^1XLR Stats: ^7$name ^7: K ^2$kills ^7D ^3$deaths ^7Ratio ^5$ratio ^7Skill ^3$skill',
        'cmd_xlrtopstats': '^1# $number: ^7$name ^7: Skill ^3$skill ^7Ratio ^5$ratio ^7Kills: ^2$kills',
    }
    ...
    def onStartup(self):
        ...
        self.console.say('^1XLRSTATS Plugin Loaded.')
    ...
    def checkMinPlayers(self, _roundstart=False):
        ...
        if self._current_nr_players < self.min_players and self._xlrstats_active and _roundstart:
            self.info('^1XLRstats disabled: not enough players online')
            if not self.silent:
            ...
            self.info('^1XLRstats enabled: collecting Stats')
            if not self.silent:
                self.console.say('^1XLRstats enabled: now collecting stats!')
